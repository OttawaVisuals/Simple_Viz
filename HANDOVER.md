...
- **`pizza-area.html`** ("One big pizza, or two smaller ones?") — `A = πr²`. A big-pizza diameter slider plus a small-pizza-diameter pill picker, with the hero drawing all circles at true relative *area* (not diameter) scale — the honesty of that scaling is the whole page. Verified presets: two 12″ (226.2 sq in) vs one 18″ (254.5 sq in, +12.5%); two 10″ vs one 16″ (+28.0%); two 14″ vs one 20″ (+2.0%, near-crossover) — all three chosen because the pair's diameter *sum* exceeds the single pizza's diameter, so the area win is the counterintuitive part.
...
- **`microwave-chocolate.html`** ("Measuring light speed with chocolate") — `c=fλ`, the real classroom experiment (turntable removed, standing-wave melted spots in chocolate/cheese). Verified λ/2≈6.12cm at the standard household 2.45GHz. Reuses `eratosthenes-shadow.html`'s "measure → estimate → compare to true value" percent-error gauge pattern rather than inventing a new one.
